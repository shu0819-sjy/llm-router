...
- Types (informational in v0.2.0 CI — `continue-on-error`; not a merge blocker): `mypy app`

The mypy job may surface legacy assignment/optional narrowing debt. Prefer fixing newly introduced type errors in PRs, but CI will not fail the pipeline solely on mypy until the gate is tightened in a later release.
