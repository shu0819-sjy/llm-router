...
**Workspace root (mandatory):** `llm-router`  
...
| Any `C:\` path usage | All code, data, Docker volumes under `llm-router` |
...
```bash
docker compose -f "llm-router\docker-compose.yml" up --build -d
```
...
**Constraint:** image build context = `llm-router` only.
...
- [ ] All artifacts under `llm-router`
